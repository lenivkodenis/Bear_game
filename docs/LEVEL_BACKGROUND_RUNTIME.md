...
Current asset quality note: the 10 per-level `background.png` files are still
the distinct approved location compositions, but they have been audited as
containing a baked-in player bear cub. They must be replaced in place with clean
environment-only versions; runtime should continue to reference the per-level
paths rather than a shared fallback.
