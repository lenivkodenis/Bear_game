...
import 'components/ambient/level_ambient_effects.dart';
...
class BearMathGame extends FlameGame with HasKeyboardHandlerComponents {
  ...
  PlayerProgress _roundStartProgress = PlayerProgress.initial();
  GameDifficulty _difficulty = GameDifficulty.beginner;
  RoundSettings _roundSettings = RoundSettings.defaults;
  ...
  Question? get currentQuestion {
    ...
    if (level == null || _currentQuestionIndex >= totalQuestions) {
      return null;
    }
  ...
  int get totalQuestions {
    final levelQuestionCount = currentLevel?.questions.length ?? 0;
    return math.min(levelQuestionCount, _roundSettings.roundQuestionCount);
  }

  int get roundMistakeCount => _roundMistakeCount;
  ...
  bool get isManualAnswerMode => usesManualAnswerInput(_difficulty);
  ...
  List<int> get currentAnswerOptions {
    ...
    return answerOptionsFor(question);
  }

  String hintFor(Question question) {
    return answerHintForDifficulty(question, _difficulty);
  }
  ...
  Future<QuestionAnswerResult> _saveCorrectAnswer(Question question) async {
    ...
    return QuestionAnswerResult(
      isCorrect: true,
      message: 'Верно! Медвежонок получает $actualEarnedSnowflakes снежинку.',
      score: newScore,
  ...
  Future<QuestionAnswerResult> _saveWrongAnswer(Question question) async {
    ...
    if (_roundMistakeCount > _roundSettings.maxMistakesPerRound) {
      ...
      return QuestionAnswerResult(
        ...
        restartRequiredReason: RestartRequiredReason.tooManyMistakes,
      );
    }
    ...
    if (_progress.score < _roundSettings.wrongAnswerPenalty) {
      ...
      return QuestionAnswerResult(
        ...
        score: _progress.score,
        isLevelComplete: false,
        restartRequiredReason: RestartRequiredReason.notEnoughSnowflakes,
      );
    }

    final newScore = GameEconomy.scoreAfterWrongAnswer(
      score: _progress.score,
      penalty: _roundSettings.wrongAnswerPenalty,
    );
    ...
    final message = hintsEnabled
        ? 'Пока неверно. Списали ${_roundSettings.wrongAnswerPenalty} снежинки. Подсказка: $hint'
        : 'Пока неверно. Списали ${_roundSettings.wrongAnswerPenalty} снежинки. Попробуй ещё раз.';
