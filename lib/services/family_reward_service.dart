...
class FamilyRewardService {
  ...
  Future<void> saveRewards(List<FamilyReward> rewards) async {
    final validatedRewards = _validatedRewards(rewards);
    final preferences = await SharedPreferences.getInstance();
    await preferences.setString(
      _rewardsKey,
      jsonEncode(validatedRewards.map((reward) => reward.toJson()).toList()),
    );
  }
  ...
  Future<void> resetRewards() async {
    ...
    await preferences.setString(
      _rewardsKey,
      jsonEncode(
        FamilyReward.defaultRewards.map((reward) => reward.toJson()).toList(),
      ),
    );
    await preferences.remove(_activeRewardIdKey);
  }
  ...
  Future<void> saveActiveReward(FamilyReward reward) async {
    ...
    for (final currentReward in rewards) {
      if (currentReward.id == reward.id) {
        updatedRewards.add(reward.validated());
  ...
  List<FamilyReward> _validatedRewards(List<FamilyReward> rewards) {
    if (rewards.isEmpty) {
    ...
    final validatedRewards =
        rewards.map((reward) => reward.validated()).toList()
          ..sort((left, right) {
            final snowflakeComparison = left.requiredSnowflakes.compareTo(
              right.requiredSnowflakes,
            );
            if (snowflakeComparison != 0) {
              return snowflakeComparison;
            }
